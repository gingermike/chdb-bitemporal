...
def _convert_nat_to_none(df: pd.DataFrame, column: str) -> pd.DataFrame:
    """Convert pd.NaT to None in a column for proper NULL handling in chdb."""
    df = df.copy()
...
def compute_changes(
    ...
    current_state_clean = _convert_nat_to_none(current_state, "as_of_to")

    # Create session and temp tables
    session = chdb.session.Session()

    # Detect column types from DataFrames
    def get_clickhouse_type(dtype, col_name: str, df: pd.DataFrame) -> str:
    ...
    id_col_defs = ", ".join([
        f"{c} {get_clickhouse_type(current_state[c].dtype, c, current_state)}"
        for c in id_columns
    ])
    value_col_defs = ", ".join([
        f"{c} {get_clickhouse_type(current_state[c].dtype, c, current_state)}"
    ...
    session.query(f"""
        CREATE TEMPORARY TABLE current_state (
            {id_col_defs}, {value_col_defs},
            effective_from Date, effective_to Date,
            as_of_from Date, as_of_to Nullable(Date)
        )
    """)
    session.query(f"""
    ...
    # Insert data using Python() table function
    session.query("INSERT INTO current_state SELECT * FROM Python(current_state_clean)")
    session.query("INSERT INTO updates SELECT * FROM Python(updates)")
